...
outputFolder2 = '../Output/MFAdaptiveTrapNoAM-del';
...
for k = 1:nMax-1
  XerrorFE(k) = norm((XFE{k}(:,end) - XFE{end}(:, end)), inf);
  ...
  comErrorFE(k) = norm(sqrt(sum(((YFE{k}(6:7, end) - YFE{end}(6:7, end))) .^ 2, 1)), inf);
end
% Trapezoidal error
XerrorTrap = norm((XTrap(:, end) - XFE{end}(:, end)), inf);
...
comErrorTrap = norm(sqrt(sum(((YTrap(6:7, end) - YFE{end}(6:7, end)) ).^ 2, 1)), inf);
