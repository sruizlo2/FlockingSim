...
% Input (predator as a repulsive potential)
ro = [0 0]; % Center of obstacle
vr = 5; % Radius or rotation
wo = pi; % Angular velocity in rad / s
lo = 1; % Width oif obstacle
beta = 2; % Shape parameter of obstacle potential
Co = 100; % Strength of obstacle potential
...
switch test_indx
  ...
  case 2
    ...
    circle_radius = 15 * rand(n_birds, 1);
...
parms = struct('n_birds', n_birds, 'Ca', Ca, 'Cr', Cr, 'lr', lr,...
  ...
  'fov', fov, 'markersize', markersize, 'a', a_start,...
  'ro', ro, 'lo', lo, 'Co', Co, 'beta', beta, 'vr', vr, 'wo', wo); % Ploting options
