function [y, m] = var_fast(x,w,dim,flag,n)
%VAR Variance.
...
if isequal(x, []) && (isequal(w,0) || isequal(w,1)) && ~hasDim
    y = real(NaN('like',x));
    m = y;
    return;
end
...
%     n = mysize(x,dim);
    if ~omitnan
        ...
        m = sum(x,dim)./n; % calculate mean
        % abs guarantees a real result
        y = sum(abs(x - m).^2, dim) ./ denom; 
        ...
        m = (sum(x, dim, 'omitnan')./n); % calculate mean
        xs = abs(x - m).^2;
    ...
    if ~isvector(w) || ~isreal(w) || ~isfloat(w) || ...
       (omitnan && any(w < 0)) || (~omitnan && ~all(w >= 0))
    ...
    if ~omitnan
        ...
        m = sum(w .* x, dim); % calculate mean
        y = sum(w .* abs(x  - m).^2, dim); % abs guarantees a real result
        ...
        m = sum(w .* x, dim, 'omitnan') ./ denom; % calculate mean
        x = x - m;
        wx2 = w .* abs(x).^2;
