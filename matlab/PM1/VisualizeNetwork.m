...
for i = 1:size(FlowValues,1),
   ...
   thickness = max(.1,abs(FlowValues(i)));
   if FlowValues(i) >= 0,
      LineStyle = '-';
   else
      LineStyle = '--';
   end
   DrawEdge(nodeA,nodeB,thickness,color,height,LineStyle);
end
...
%*****************************************************
function DrawEdge(nodeA,nodeB,thickness,color,height,LineStyle);
...
line(X,Y,Z, 'LineWidth', thickness, 'color', color, 'LineStyle', LineStyle);
